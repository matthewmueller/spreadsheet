...
function Cell(value, at, spreadsheet) {
  ...
  // this.events.bind('click', 'edit');

  // add shortcuts
  // this.shortcuts = shortcuts(this.input, this);
  // this.shortcuts.k.ignore = false;
  // this.shortcuts.bind('esc', 'blur');
...
Cell.prototype.val = function(val, opts) {
  ...
  this.input.value = opts.compute ? this.compute(val) : val;
  this.value = val;
...
Cell.prototype.onkeydown = function(e) {
  ...
  if (!classes.has('editing') && classes.has('editable')) {
    input.focus();
    input.value = '';
  }
...
Cell.prototype.blur = function() {
  this.classes.remove('highlighted').remove('focused').remove('editing');
