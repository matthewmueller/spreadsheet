...
var spreadsheet = domify('<div class="spreadsheet"><table><thead></thead><tbody></tbody></table></div>');
...
function Spreadsheet() {
  ...
  this.rowheads = [];
  this.rowheads.push(function(i) {
    return domify('<th class="rowhead">' + i + '</th>');
  });

  this.colheads = [];
  this.colheads.push(function(i) {
    return domify('<th class="colhead"><div>' + ntol(i) + '</div></th>');
  });
}
...
Spreadsheet.prototype.insert = function(cells) {
  ...
  var biggest = largest(this.cells);
  this.fill(biggest);
...
Spreadsheet.prototype.draw = function(rows, cols) {
  var spreadsheet = this.spreadsheet;
  var rowheads = this.rowheads;
  var filler = rowheads.length;
  ...
  for (var i = 0, colhead; colhead = colheads[i]; i++) {
    if (!trs[i]) thead.appendChild(document.createElement('tr'));
    var ths = trs[i].getElementsByTagName('th');

    for (var j = ths.length; j < cols + filler; j++) {
      if (j < filler) {
        var th = document.createElement('th');
        th.className = 'filler';
        trs[i].appendChild(th);
      } else {
        trs[i].appendChild(colhead(j - filler));
      }
    }
  }
  ...
  for (var i = 0; i < rows; i++) {
    if (!trs[i]) tbody.appendChild(document.createElement('tr'));
    var ths = trs[i].getElementsByTagName('th');
    var tds = trs[i].getElementsByTagName('td');

    for (var j = ths.length + tds.length; j <= cols + filler; j++) {
      if (j < filler) {
        var th = rowheads[j](i);
        trs[i].appendChild(th);
      } else {
        var letter = ntol(j - filler);
        var at = letter + (i + 1);
        var cell = spreadsheet[at] || new Cell(null, at, this);
        spreadsheet[at] = cell;
        trs[i].appendChild(cell.render());
      }
    }
  }
}
...
Spreadsheet.prototype.fill = function(cell) {
  ...
  return this.draw(rows, cols);

  // fill in column headers
  var colhead = thead.getElementsByTagName('tr')[0];
  colhead = colhead ? colhead : domify('<tr><th class="rowhead"></th></tr>');
  var ths = colhead.querySelectorAll('.colhead');
  for (var i = 0; i <= cols; i++) {
    ...
    if (!th) {
      ...
      // TODO: see if there is a way to do this in pure CSS
      th.className += !i ? ' first' : '';
      th.setAttribute('name', ntol(i));
  ...
  for (var i = 1; i <= rows; i++) {
    ...
    if (!tr) {
      ...
      rh.innerHTML = '<div>' + i + '</div>';
      tr.appendChild(rh);

      rh = domify('<th class = "layer"><div></div></th>')
      tr.appendChild(rh);
    }
