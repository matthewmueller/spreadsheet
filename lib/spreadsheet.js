...
Spreadsheet.prototype.merge = function(cells) {
  ...
  var largestCell = largest(cells);
  var diff = subtract(largestCell.at, captain.at);
  captain.attr('rowspan', diff.row + 1);
  captain.attr('colspan', diff.col + 1);

  // insert and fill, even if null
  this.insert(captain);
  this.fill(largestCell.at);

  return this;
}

/**
 * Check if the referenced cell is an alias
 * of a merged cell
 */

Spreadsheet.prototype.alias = function(cell) {
